...
router.post('/apply', verifyJWT, async (req, res) => {
  try {
    ...
    const personalizedLink = `https://localhost:5173/link/${trackingId}`;
...
router.get('/link/api/:uid', async (req, res) => {
  try {
    const { uid } = req.params;
    const details = await Detail.find({Uid: uid});
    ...
    const user = await Student.find({name:details.Name});
